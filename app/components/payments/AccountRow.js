...
class AccountRow extends React.PureComponent {
  ...
  render() {
    const { account = {}, clzzName, onHandleClick } = this.props;
    ...
    return (
      <ApolloConsumer>
        {client => {
          ...
          return (
            <tr onClick={() => onHandleClick(account.id)} className={clzzName}>
              <td>{account.id}</td>
              ...
              <td>
                {contentAdd.state}, {contentAdd.country}
              </td>
