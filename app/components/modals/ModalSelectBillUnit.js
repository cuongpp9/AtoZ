...
class ModalSelectBillUnit extends React.Component {
  constructor() {
    super();
  ...
  renderBody(data) {
    ...
    if (!data.length) {
      return (
        <tbody>
          <tr>
            <td colSpan={8}>{`No Bill Unit has found for account ${accountId}`}</td>
          </tr>
        </tbody>
      );
    }
  ...
  render() {
    ...
    return (
      <Modal
        ...
        className="modal-custom"
      >
        <ModalHeader toggle={() => toggleModal()}>Choose Bill Unit:</ModalHeader>
        <ModalBody>
...
export default ModalSelectBillUnit;
