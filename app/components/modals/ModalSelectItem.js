...
import { FilterItemId } from 'components/filterselect';

const headers = ['Id', 'Name', 'Description', 'Company', 'Status', ''];
...
class ModalSelectParentItem extends React.Component {
  ...
  renderRow = (row, id) => (
    <tr
      ...
      className={className({
        'column-active': this.props.idSelected === row.id,
      })}
    >
      <td>{row.id}</td>
      <td>{row.name}</td>
      ...
      <td>{row.status}</td>
      <td style={{ width: 55 }}>
        {this.props.idSelected && this.props.idSelected === row.id ? (
        ...
        <ModalBody>
          ...
          <CardBody>
            <table className="table table-bordered">
              ...
              {this.renderBody(
                _.uniqBy(
                  itemId ? items.filter(el => el.id !== itemId) : items,
                  'id',
                ),
              )}
            </table>
