...
const heads = [
  ...
  {
    key: 'numberInvoicesInCollection',
    name: 'Num of Inoices in Collection',
...
class TableSearchAccounts extends Component {
  ...
  handleClickRow = row => {
    const location = {
      pathname: '/collections/collection-agent/invoice-units',
  ...
  renderRow = (row, id) => (
    <tr key={id} onClick={() => this.handleClickRow(row)}>
      <td>{row.accountId}</td>
      <td>{row.firstName}</td>
      <td>{row.lastName}</td>
