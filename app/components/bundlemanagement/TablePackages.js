...
class TablePackages extends Component {
  ...
  handleSort = sortName => {
    ...
    this.props.handleSortPackages(keySort, valueSort);
  };

  handleClickRow = id => {
    this.props.history.push(`/bundle-management/packages/${id}/detail`);
  };

  renderRow = row => (
    <tr key={row.id} onClick={() => this.handleClickRow(row.id)}>
      <td>{row.id}</td>
      <td>{row.name}</td>
...
TablePackages.propTypes = {
  ...
  handleSortPackages: PropTypes.func,
...
export default compose(withRouter)(TablePackages);
